...
class Actor(nn.Module):
    ...
    def sample_act(self, s):
        s = torch.from_numpy(s).float().to(device)
        actions_probs = F.softmax(self.actor_mlp(s), dim=-1)
...
class DiscreteSACAgent:
    def __init__(self, state_dim, action_dim, n_hidden_units, lr=0.001, memory_size=50000, gamma=0.99, target_entropy_ratio=0.4,
                 update_interval=1, target_update_interval=300, num_eval_steps=1000, max_episode_steps=300, eval_interval=10,
...
def main():
    ...
    sac = DiscreteSACAgent(state_dim, action_dim, n_hidden_units, lr=lr, gamma=gamma, tau=tau, memory_size=memory_size, update_interval=learn_every_n_steps)
